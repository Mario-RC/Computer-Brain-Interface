...
function [mean_conv] = media_conv_vel(i,trial_data,idx,start,finish,gaussFilter)

suma = 0;

for j = idx'
    W = conv(trial_data(j).vel(:,1), gaussFilter, 'same');
    Z = conv(trial_data(j).vel(:,2), gaussFilter, 'same');
